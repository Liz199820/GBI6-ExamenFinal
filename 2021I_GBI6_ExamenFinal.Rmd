...
descr(df_long_1, out = "viewer", file = "results/mRNA_expr_summary.doc")
```
...
plot <- LG %>% ggplot(aes(x = gen, y = expresion_level, color= continent)) + geom_boxplot() + geom_jitter(alpha=0.9, size=0.5)} 
```
...
ren_gen_expression <- function(gen){
GS <- df_long %>% filter(dataset == GATA3)
} geom_point(mapping = aes(x = GATA3, y = FN1)
```
